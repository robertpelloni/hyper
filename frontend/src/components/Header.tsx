    ...
    <div className={`tn_header ${isMac ? 'tn_header_mac' : ''}`}>
      ...
      {!isMac && (
        <div ref={hamburgerRef} style={{position: 'relative'}}>
          <div className="tn_hamburger_btn" onClick={() => setShowHamburger(!showHamburger)} title="Menu">
            ☰
          </div>
          {showHamburger && (
            <div className="tn_profile_dropdown" style={{left: 0, top: '34px'}}>
              <div
              ...
              <div className="tn_profile_item" onClick={() => setShowHamburger(false)}>
                About TormentNexus
              </div>
            </div>
          )}
      ...
      {/* New Tab Button */}
      <div className="tn_new_tab_btn" onClick={onNewTab} title="New Tab (Ctrl+Shift+T)">
        +
      </div>
      ...
      {!isMac && (
        <div className="tn_window_controls">
          ...
          <div className="tn_window_btn" onClick={onMaximise} title={maximized ? 'Restore' : 'Maximize'}>
            {maximized ? '❐' : '□'}
          </div>
