				...
				navigator.clipboard
					...
					.then((text) => {
						if (text) {
							...
							const escaped = session.shell?.endsWith("cmd.exe")
								? `"${text}"`
								: `'${text.replace(/'/g, `'\\''`)}'`;
