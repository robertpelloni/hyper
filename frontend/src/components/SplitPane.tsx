...
export default function SplitPane({direction, children, sizes, onResize, borderColor}: SplitPaneProps) {
          ...
          {i < childArray.length - 1 && (
            <div
              ...
              onMouseDown={(e) => {
                ...
                const containerSize = direction === 'vertical' ? container.clientWidth : container.clientHeight;

                const handleMove = (moveEvent: MouseEvent) => {
                  const delta = direction === 'vertical' ? moveEvent.clientX - startX : moveEvent.clientY - startY;
                  const ratio = delta / containerSize;

                  const newSizes = [...defaultSizes];
                  const sum = newSizes[i] + newSizes[i + 1];
                  newSizes[i] = Math.max(0.1, Math.min(0.9, newSizes[i] + ratio));
