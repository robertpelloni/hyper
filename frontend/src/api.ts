...
import type {AppConfig, SessionInfo, AgentStatus, MCPTool, MCPTelemetry} from './types';
...
export async function writeSessionData(uid: string, data: string): Promise<void> {
  return SessionBindings.WriteData(uid, data);
}

export async function resizeSession(uid: string, rows: number, cols: number): Promise<void> {
...
// ===== Agent =====
export async function agentExecuteCommand(command: string): Promise<AgentStatus> {
